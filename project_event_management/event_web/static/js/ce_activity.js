document.addEventListener('DOMContentLoaded', function() {
    ...
    uploadInput.addEventListener('change', function (event) {
        ...
        if (totalUploadedImages > maxImages) {
            alert(`You uploaded ${totalUploadedImages} images but You can only upload a maximum of ${maxImages} images in total.`);
...
function confirmSubmit(action, activityId = null) {
    ...
    if (confirm(message)) {
        if (action === 'create') {
            ...
            window.location.href = "{% url 'url_mo_editactivity' activity_id=1 %}".replace("1", activityId);
        }
        return true;
    }
    return false;
